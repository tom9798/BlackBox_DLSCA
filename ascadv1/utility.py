...
file = open(os.path.join(_THIS_DIR, 'utils', 'dataset_parameters'), 'rb')
...
class SharedWeightsDenseLayer(tf.keras.layers.Layer):
    def __init__(self, input_dim=1,units = 1, shares = 16,name = '',activation = True, seed = 42):
        if name == '':
...
class GF256MultiplyLayer(tf.keras.layers.Layer):
    def __init__(self, classes=256, name='', seed=42):
        super(GF256MultiplyLayer, self).__init__(name=name)
        self.classes = classes
        
        # Generate GF(2^8) multiplication table for AES polynomial 0x11B
        mul_table = np.zeros((classes, classes), dtype=np.int32)
        for i in range(classes):
            for j in range(classes):
                # GF Multiplication logic
                p = 0
                a = i
                b = j
                for _ in range(8):
                    ...
                    a = (a << 1) & 0xFF
                    if hi_bit_set:
                        a ^= 0x1B # 0x11B field polynomial
                    b >>= 1
                mul_table[i, j] = p
        
        self.mul_table = mul_table
        
        # Generate Inversion Table: inv_table[i] = i^-1
        inv_table = np.zeros(classes, dtype=np.int32)
        for i in range(classes):
            if i == 0: 
                inv_table[i] = 0 # 0 maps to 0
                continue
            for j in range(classes):
                # Check if i * j = 1
                if mul_table[i, j] == 1:
        ...
        # Construct Permutation Table M[i, k] = k * inv(i) (for i != 0)
        # This table answers: "If A=i, which B yields A*B=k?" => B = k*inv(i)
        perm_table = np.zeros((classes, classes), dtype=np.int32)
        for i in range(classes):
            for k in range(classes):
                if i == 0:
                    perm_table[i, k] = 0 # Dummy
                else:
                    inv_i = inv_table[i]
                    # mul(k, inv_i)
                    p = mul_table[k, inv_i]
    ...
    def call(self, inputs):
        # inputs[0] = P(A), inputs[1] = P(B)
        pa = inputs[0] # (batch, 256)
        pb = inputs[1] # (batch, 256)
        
        # Gather permuted B: for each i, k, get P(B = k * i^-1)
        # resulting shape: (batch, 256_i, 256_k)
        # At [b, i, k], we have Prob(B = perm_table[i, k])
        pb_permuted = tf.gather(pb, self.perm_table, axis=1) 
        
        # Compute contributions for i != 0: P(A=i) * P(B = k*i^-1)
        # We need to mask out i=0 first because perm_table[0,:] is dummy.
        
        valid_i_mask = np.ones((self.classes, 1), dtype=np.float32)
        valid_i_mask[0, 0] = 0.0
        valid_i_mask = tf.constant(valid_i_mask) 
        
        pa_masked = pa * tf.transpose(valid_i_mask) # (batch, 256)
        
        # Sum over i to get P(C=k)
        res_nonzero = tf.einsum('bi,bik->bk', pa_masked, pb_permuted)
        
        # Handle i = 0 separately
        # If A=0, then C=0 always. P(C=0) += P(A=0).
        pa_zero = pa[:, 0:1] # (batch, 1)
        
        # Create a tensor that allows adding to index 0 only
        # We want to add P(A=0) to result column 0.
        zero_contrib = tf.pad(pa_zero, [[0,0], [0, 255]]) 
        
        final_res = res_nonzero + zero_contrib
        return final_res
    ...
    def get_config(self):
        ...
        base_config = super(GF256MultiplyLayer, self).get_config()
        return dict(list(base_config.items()) + list(config.items()))


class GF256InvertLayer(tf.keras.layers.Layer):
    def __init__(self, classes=256, name='', seed=42):
        super(GF256InvertLayer, self).__init__(name=name)
        self.classes = classes
        
        # Table generation logic repeated (safe/isolated) or reused if refactored
        # Ideally, we should refactor table gen, but for safety in this edit, self-contained is fine.
        inv_table = np.zeros(classes, dtype=np.int32)
        
        # Helper for GF Mult just for this block if needed, or assume we can compute brute force?
        # Actually brute force inverse is annoying without the table.
        # Let's generate mul table first again briefly or use a smarter way.
        # Smarter way: reuse logic
    ...
    def call(self, inputs):
        # input: Dist(X) (batch, 256)
        # output: Dist(X^-1)
        # P(Y=k) = P(X = k^-1)
        # Output[k] comes from Input[inv(k)]
        return tf.gather(inputs, self.inv_table, axis=1)

    def get_config(self):
        config = {
            'classes': self.classes
        }
        base_config = super(GF256InvertLayer, self).get_config()
        return dict(list(base_config.items()) + list(config.items()))
